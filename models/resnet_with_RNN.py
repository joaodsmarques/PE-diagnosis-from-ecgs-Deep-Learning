...
class ResNet(nn.Module):
    def __init__(self, ResBlock, layer_list, num_classes, num_channels=2):
        ...
        #The first parameter is the number of expected features/channels (not the sequence length)
        #self.rnn = nn.RNN(512, self.hidden_size, self.num_layers, batch_first=True, dropout = 0.2)
        #self.gru = nn.GRU(512, self.hidden_size, self.num_layers, batch_first=True, dropout = 0.2)
        self.lstm = nn.LSTM(512, self.hidden_size, self.num_layers, batch_first=True, dropout = 0.25)
    ...
    def forward(self, x):
        ...
        #RNN needs - seq_length, batch_size, features 
        #can use batch first also to keep the organization
        #print (x.shape)
        ...
        #We just need the result from the last iteration
        out = out[:, -1, :]
...
def ResNet18_with_RNN(num_classes, channels=12):
